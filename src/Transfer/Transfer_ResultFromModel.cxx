...
Handle(TColStd_HSequenceOfTransient) Transfer_ResultFromModel::TransferredList(const Standard_Integer level) const
{
  Standard_Integer i, nb;
  Handle(TColStd_HSequenceOfTransient) list = new TColStd_HSequenceOfTransient();
...
Handle(TColStd_HSequenceOfTransient) Transfer_ResultFromModel::CheckedList(const Interface_CheckStatus check, const Standard_Boolean result) const
{
  Standard_Integer i, nb;
  Handle(TColStd_HSequenceOfTransient) list = new TColStd_HSequenceOfTransient();
...
Interface_CheckIterator Transfer_ResultFromModel::CheckList(const Standard_Boolean erronly, const Standard_Integer level) const
{
  ...
  for (i = 1; i <= nb; i++)
  {
    ...
    if (stat == Interface_CheckOK ||
        (stat == Interface_CheckWarning && erronly))
      continue;
